...
city_coordinates_sf <- geocode(EPO_sites_tbl$Site) %>% 
    mutate(Site = EPO_sites_tbl$Site) %>% 
...
server <- function(input, output) {
    ...
    output$map <- renderLeaflet({
        filtered_data <- final_table_no_staff_site_coordinates %>%
            filter(Year == input$year_slider)
        
        leaflet(filtered_data) %>%
            addTiles(urlTemplate = "https://tile.thunderforest.com/mobile-atlas/{z}/{x}/{y}.png?apikey=c6b58c4da56a4abf88672b3f82ef275d",
            ...
            addCircleMarkers(
                ...
                lng = ~lon, lat = ~lat,
                label = paste(
                    "<strong>", filtered_data$Site,
                    "</strong><br>Staff number:", filtered_data$`Staff number`) %>%
