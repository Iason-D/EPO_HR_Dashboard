...
vector_text <- here("Data", "social_report_2016_en.pdf") %>% 
    pdf_text() %>% 
    str_split("\n") %>% 
    unlist()
...
table_nationality_compare_2015_to_2022 <- function(raw){
    ...
    data_table <- read.csv(text_con, sep = "|", header = F) %>% 
        ...
        mutate(Country = c("Albania", "Austria", "Belgium", "United Kingdom", "Bulgaria", "Croatia",
                           ...
                           "France", "Germany", "Greece", "Hungary", "Iceland", "Republic of Ireland", "Italy",
...
table_general_welfare <- function(raw){
    ...
    data_table <- data_table %>% 
        ...
        mutate(Category = case_when(
            Category == "Social events Office" ~"Social Events office",
            Category == "Pension association subsidy"     ~ "Pensioners' Association subsidy",
            Category == "Pensioners’ association subsidy" ~ "Pensioners' Association subsidy",
            Category == "Welfare Miscellaneous"  ~ "Welfare miscellaneous",
            Category == "Welfare, miscellaneous" ~ "Welfare miscellaneous",
            Category == "AMICALE, culture and sport clubs, social events" ~ "AMICALE, culture and sports clubs, social events",
            TRUE ~ Category) 
        ) #%>% 
...
table_conflicts <- function(raw){
    ...
    data_table <- read.csv(text_con, sep = "|", header = F) %>%
        ...
        mutate(Conflict_type = case_when(
            str_detect(Conflict_type, "the hierarchical line") ~ "Manager employee",
            str_detect(Conflict_type, "the hierarchical line") ~ "Manager employee",
            str_detect(Conflict_type, "Managers/employees in the") ~ "Manager employee",
            Conflict_type == "Private matter"           ~ "Private matters",
            Conflict_type == "Private matters"          ~ "Private matters",
            Conflict_type == "Manager and manager"      ~ "Between managers",
            Conflict_type == "Between managers"         ~ "Between managers",
            Conflict_type == "Between colleagues"       ~ "Between colleagues",
            Conflict_type == "Requests for information" ~ "Request for information",
            TRUE ~Conflict_type)) #%>% 
...
final_table_conflicts <- map_dfr(years, extract_table_conflicts) %>% 
...
# Use map to process raw_text_list, extract tables for each year and rbind them
final_table_no_staff_type <- map_dfr(years, extract_table_no_staff_type) %>% 
...
# Use map to process raw_text_list, extract tables for each year and rbind them
final_table_no_staff_site <- map_dfr(years, extract_table_no_staff_site) %>% 
    pivot_longer(cols = c("Male", "Female"), names_to = "Gender", values_to = "Gender staff") %>% 
    mutate(Year = as.numeric(Year)) %>% 
    #mutate(Year = ISOdate(Year, 1, 1)  %>% as_date()) %>% 
    arrange(Year) 




# The chunk that follows assigns coordinates to the five EPO sites and adds them to the final_table_no_staff_site
# EPO_sites
EPO_sites_tbl <- tibble(Site = c("Munich", "The Hague", "Berlin", "Vienna", "Brussels"), 
                        Countries = c("Germany", "Netherlands", "Germany", "Austria", "Belgium"))


library(ggmap)
#Register Google Maps API key
register_google(key = "AIzaSyAS8DUb86GCXB-rRStxBdF07V7IKhFbTjw") # a ggmap function


library(sf)
# Fetch latitude and longitude for each city and create a dataframe by adding the city names
city_coordinates_sf <- geocode(final_table_no_staff_site$Site) %>% 
    st_as_sf(coords = c("lon", "lat"), 
             remove = FALSE,
             crs = 4326)

# Add the city coordinates to the final_table_no_staff_site by doing a left_join()
final_table_no_staff_site_coordinates <- final_table_no_staff_site %>% 
    mutate(Site = str_remove(Site, "^\\s")) %>% 
    cbind(city_coordinates_sf) %>% 
    group_by(Year) %>% 
...
# Function to extract and process table for a given year
extract_table_nationality_compare_2015_to_2022 <- function(year) {
    index <- which(years == year)
    raw_text <- pluck(raw_text_list[3:10], index)
    
    # Extract the table using the custom function
    table_data <- table_nationality_compare_2015_to_2022(raw_text) 
...
# Use map to process raw_text_list, extract tables for each year and rbind them
final_table_nationality_compare_2015_to_2022 <- map_dfr(years, extract_table_nationality_compare_2015_to_2022) %>% 
     bind_rows() %>% 
     mutate(Year = as.numeric(Year))


country_coordinates_sf <- geocode(final_table_nationality_compare_2015_to_2022$Country) %>% 
    st_as_sf(coords = c("lon", "lat"),
             remove = FALSE,
             crs    = 4326)


# Add the country  coordinates to the final_table_nationality_compare_2015_to_2022 by doing a cbind()
final_table_nationality_coords <- final_table_nationality_compare_2015_to_2022 %>% 
    mutate(Nationality = str_remove(Nationality, "^\\s")) %>% 
    cbind(country_coordinates_sf)
...
# Use map to process raw_text_list, extract tables for each year and rbind them
final_table_applications_per_vacancy_2017_to_2022 <- map_dfr(years, extract_table_applications_per_vacancy_2017_to_2022) %>% 
    distinct() %>% 
    mutate(Year = as.numeric(Year)) %>% 
    arrange(Year) 
...
# Use map to process raw_text_list, extract tables for each year and rbind them
final_table_talent <- map_dfr(years, extract_table_talent) %>% 
    mutate(Year = as.numeric(Year)) %>% 
    arrange(Year) 
...
# Use map to process raw_text_list, extract tables for each year and rbind them
final_table_general_welfare <- map_dfr(years, extract_table_general_welfare) %>% 
    arrange(Category) %>% 
    mutate(Year = as.numeric(Year)) %>% 
    arrange(Year) 
...
# Use map to process raw_text_list, extract tables for each year and rbind them
final_table_accidents <- map_dfr(years, extract_table_accidents) %>% 
...
# Use map to process raw_text_list, extract tables for each year and rbind them
final_table_service_ending <- map_dfr(years, extract_table_service_ending) %>% 
    mutate(Year = as.numeric(Year)) %>% 
...
# Use map to process raw_text_list, extract tables for each year and rbind them
final_table_basic_salaries_2016_to_2022 <-  map_dfr(years, extract_table_basic_salaries_2016_to_2022) %>% 
    #mutate(Year = ISOdate(Year, 1, 1)  %>% as_date()) %>% 
    mutate(Year = as.numeric(Year)) %>% 
    arrange(Year) 
...
ui <- fixedPage(
    ...
    tabsetPanel(
        tabPanel("EPO Staff numbers by type",
                 plotlyOutput("plot_staff_type", width = "800px"),
                 #plotlyOutput("plot_staff_site"),
                 plotlyOutput("plot_staff_type_gender", width = "800px"),
                 plotlyOutput("plot_staff_site_gender", width = "800px")),
        tabPanel("EPO Staff numbers by site",
                 leafletOutput("map", height = "800px"),
                 sliderInput("year_slider", "Select Year",
                             min = min(final_table_no_staff_site_coordinates$Year),
                             max = max(final_table_no_staff_site_coordinates$Year),
                             value = min(final_table_no_staff_site_coordinates$Year),
                             step = 1, 
                             sep="")),
        tabPanel("EPO Staff numbers by nationality",
                 leafletOutput("Map", height = "800px"),
                 sliderInput("Year_slider", "Select Year",
                             min = min(final_table_nationality_coords$Year),
                             max = max(final_table_nationality_coords$Year),
                             value = min(final_table_nationality_coords$Year),
                             step = 1, 
                             sep="")),
        tabPanel("Salaries",
                 plotlyOutput("plot_salaries", width = "800px"),
                 plotlyOutput("plot_salary_shares", width = "800px")),
        tabPanel("End of Service",
                 plotlyOutput("plot_service_end", width = "800px"),
                 plotlyOutput("plot_pc_service_end", width = "800px")),
        tabPanel("Applications", plotlyOutput("plot_applications", width = "800px")),
        tabPanel("Accidents",
                 plotlyOutput("plot_accidents", width = "800px"),
                 plotlyOutput("plot_pc_accidents", width = "800px")),
        tabPanel("Conflicts", plotlyOutput("plot_conflict", width = "800px")),
        tabPanel("Welfare", plotlyOutput("plot_welfare", width = "800px"))
...
server <- function(input, output) {
    ...
    output$map <- renderLeaflet({
        filtered_staff_data <- final_table_no_staff_site_coordinates %>%
            filter(Year == input$year_slider)
        
        leaflet(filtered_staff_data) %>%
            addTiles(urlTemplate = "https://tile.thunderforest.com/mobile-atlas/{z}/{x}/{y}.png?apikey=a7ffdba98e184bedb714940b381bf569",
            options = tileOptions(maxZoom = 6.5, minZoom = 5.5)) %>%
            setView(lng = 10, lat = 50, zoom = 5.5) %>%
            addCircleMarkers(
                data = filtered_staff_data,
                lng = ~lon, lat = ~lat,
                label = paste(
                    "<strong>", filtered_staff_data$Site,
                    "</strong><br>Staff number:", filtered_staff_data$`Staff number`) %>%
                    lapply(htmltools::HTML),
                labelOptions = labelOptions(noHide = TRUE),
                radius = ~sqrt(`Staff number`) * 1,  # Adjust the multiplier to control circle size
                popupOptions = popupOptions(closeButton = FALSE)
            )
    })
    output$Map <- renderLeaflet({
        filtered_nationality_data <- final_table_nationality_coords %>%
            filter(Year == input$Year_slider)
        
        leaflet(filtered_nationality_data) %>%
            addTiles(urlTemplate = "https://tile.thunderforest.com/mobile-atlas/{z}/{x}/{y}.png?apikey=a7ffdba98e184bedb714940b381bf569",
                     options = tileOptions(maxZoom = 6, minZoom =4)) %>%
            setView(lng = 10, lat = 50, zoom =4.3) %>%
            addCircleMarkers(
                data = filtered_nationality_data,
                lng = ~lon, lat = ~lat,
                label = paste(
                    "<strong>", filtered_nationality_data$Nationality,
                    "</strong><br>Staff number:", filtered_nationality_data$`EPO Staff`) %>%
                    lapply(htmltools::HTML),
                labelOptions = labelOptions(noHide = F),
                radius = ~sqrt(`EPO Staff`) * 1,  # Adjust the multiplier to control circle size
                popupOptions = popupOptions(closeButton = FALSE)
                )
    })
